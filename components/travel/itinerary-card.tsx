...
export function ItineraryCard({ activity, showPrices }: ItineraryCardProps) {
  const CardWrapper = activity.url ? 'a' : 'div'
  const cardProps = activity.url ? {
    href: activity.url,
    target: "_blank",
  ...
  return (
    <CardWrapper 
      {...cardProps}
      className={`block w-full bg-[#1A1A1B] rounded-xl overflow-hidden border border-[#2A2A2B] transition-all duration-300 hover:border-[#C6A96B]/30 active:scale-[0.99] ${activity.url ? 'cursor-pointer' : ''}`}
    >
      {/* Image */}
      ...
      <div className="p-5">
        ...
        {activity.url && (
          <div className="flex items-center gap-1.5 text-xs text-[#7A7A7A] font-sans mt-3">
